...
from astronex.utils import parsestrtime, strdate_to_date
...
class Chart(object):
    ...
    def calc_plan_with_retrogression(self,epheflag=4):
        ...
        dt = NeXDate(self,dt,ZoneInfo(self.zone))
...
##############################################
    ...
    def calc_nodal_agep(self,plan):
        ...
        for i in range(12):
            ...
            events.sort(key=cmp_to_key(evcmp))
            for e in events:
                fac = e['scusp'] / 30
    ...
    def house_time_lapsus(self,h,playagain=False):
        ...
        # datetime exige 1 <= year <= 9999. Al arrastrar la biografia muy atras
        # (comportamiento original) el ano calculado puede salirse de ese rango y
        # provocar ValueError. En PyGTK2 esa excepcion solo abortaba el expose y
        # dejaba el frame previo; en GTK3 abortaria todo el dibujo (grafica + rueda).
        # Acotamos el ano para preservar el arrastre libre sin que desaparezca nada.
        y1 = max(1, min(9999, year + h * 6))
        y2 = max(1, min(9999, year + (h + 1) * 6))
        try:
            bbegin = datetime(y1,month,day,hour,minutes,0)
            eend = datetime(y2,month,day,hour,minutes,0)
        except ValueError: # leap years
            bbegin = datetime(y1,month,day-1,hour,minutes,0)
            eend = datetime(y2,month,day-1,hour,minutes,0)
        lapsus = eend - bbegin
