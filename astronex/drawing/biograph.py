...
from astronex.chart import planclass,aspclass,orbs
from astronex.drawing.roundedcharts import NodalChart,SoulChart,LocalChart,RadixChart,DharmaChart
...
boss = _LazyProxy(_boss)
curr = _LazyProxy(_curr)
    ...
    def pe_rulercb(self,child,event,surface):
        ...
        if hasattr(surface, 'opaux'):
            ...
            if not curr.curr_op.startswith('bio') or curr.opmode != 'simple' or curr.curr_chart == curr.now:
                return False
        info = getattr(child, '_move_info', None)
        if info is None:
            info = {'button': -1}
            child._move_info = info
        width = child.get_allocation().width
        # event.button NO es fiable en la senal generica 'event' de PyGObject;
        # hay que leerlo con get_button() que devuelve (ok, numero).
        ok_btn, btnnum = event.get_button() if event.type in (
            Gdk.EventType.BUTTON_PRESS, Gdk.EventType._2BUTTON_PRESS,
            Gdk.EventType.BUTTON_RELEASE) else (False, 0)
        if event.type == Gdk.EventType._2BUTTON_PRESS:
            boss.da.panel.nowbut.emit('clicked')
            info['button'] = 100
            return True
        elif event.type == Gdk.EventType.BUTTON_PRESS:
            if ok_btn and btnnum != 1:
                return False
            if info['button'] < 0:
                info['button'] = 1
            ...
            if info['button'] == 100:
                ...
                return True
            info['button'] = -1
            win = surface.get_window()
            if win is not None:
                device = win.get_display().get_default_seat().get_pointer()
                rx = win.get_device_position(device)[1]
            else:
                rx = event.x
            ruler[0] = float(rx) / width
            release = True
        elif event.type == Gdk.EventType.MOTION_NOTIFY:
            if info['button'] < 0 or info['button'] == 100:
                info['button'] = -1
                return False
            # Bajo POINTER_MOTION_HINT_MASK hay que pedir la posicion del puntero
            # para re-armar los eventos de movimiento (equivalente a event.window.get_pointer()
            # en PyGTK2). Sin esto el arrastre se queda pegado tras el primer evento.
            win = surface.get_window()
            if win is not None:
                device = win.get_display().get_default_seat().get_pointer()
                x = win.get_device_position(device)[1]
            else:
                x = event.x
