...
from astronex.drawing.roundedcharts import HouseChart
...
class ProfileMixin(object):
    ...
    def draw_prof(self,cr,w,h,chartob):
        ...
        cr.save()
        cr.scale(scl,scl)
        cr.append_path(profile)
        cr.clip()
        scl_inv = 1/scl
        cr.scale(scl_inv,scl_inv)
        ...
        for i in range(0,360,30):
            ...
            cr.line_to(radius*cusps[m]*math.cos(c*RAD),radius*cusps[m]*math.sin(c*RAD))
            cr.line_to(radius*lows[m]*math.cos(l*RAD),radius*lows[m]*math.sin(l*RAD))
            if m == 11:
                cr.close_path()
        cr.fill()
        cr.restore()
        ...
        for h in range(12):
            ...
            cr.line_to(xx,yy)
            cr.close_path()
            yellow_pts[h] = (x,y,xx,yy)
        cr.set_source_rgb(1,1,0.8)
        cr.fill()
        cr.set_source_rgb(1,0,0)
        for x,y,xx,yy in yellow_pts:
            cr.move_to(x,y)
            cr.line_to(xx,yy)
        cr.stroke()
        ...
        for h in range(12):
            ...
            n = phi - third11; m = third11
            x =  (n*x2[h] + m*x1[h])/(m+n)
            y =  (n*y2[h] + m*y1[h])/(m+n)
            cr.line_to(x,y)
            n = third22*5; m = third22
            xx =  (n*x2[h] + m*x1[(h+1)%12])/(m+n)
            yy =  (n*y2[h] + m*y1[(h+1)%12])/(m+n)
            cr.line_to(xx,yy)
            cr.close_path()
            gray_pts[h] = (x,y,xx,yy)
        cr.set_source_rgb(0.7,0.7,0.7)
        cr.fill()
        cr.set_source_rgb(0.4,0.4,0.4)
        for x,y,xx,yy in gray_pts:
            cr.move_to(x,y)
            cr.line_to(xx,yy)
        cr.stroke()
       ...
       #### axes
        ...
        for i in range(12):
            ...
            cos_off = math.cos(off*RAD); sin_off = math.sin(off*RAD)
            axis_buckets[i % 3].append((ocusp*cos_off, ocusp*sin_off, icusp*cos_off, icusp*sin_off))
        axis_colors = [(0.6,0,0), (0,0,0.6), (0,0.5,0)]
        for col, segs in zip(axis_colors, axis_buckets):
            cr.set_source_rgb(*col)
            for ax, ay, ix, iy in segs:
                cr.move_to(ax, ay)
                cr.line_to(ix, iy)
            cr.stroke()
