...
from astronex.utils import format_latitud, format_longitud
from astronex.drawing.roundedcharts import RadixChart,HouseChart,NodalChart
from astronex.drawing.aspects import SimpleAspectManager
...
class SheetMixin(object):
    ...
    def data_nodal_planh(self,cr):
        ...
        taba.set_tab(0,Pango.TabAlign.LEFT,120)
        ...
        PangoCairo.show_layout(cr, layout)
        layout.set_markup("")
        taba.set_tab(0,Pango.TabAlign.LEFT,98)
        layout.set_tabs(taba)
    ...
    def data_house_planh(self,cr):
        ...
        taba.set_tab(0,Pango.TabAlign.LEFT,100)
        taba.set_tab(1,Pango.TabAlign.LEFT,150)
        layout.set_tabs(taba)
        ...
        taba = Pango.TabArray(2,True)
        taba.set_tab(0,Pango.TabAlign.LEFT,90)
        taba.set_tab(1,Pango.TabAlign.LEFT,140)
        layout.set_tabs(taba)
        cr.set_source_rgb(0,0,0)
    ...
    def data_planh(self,cr,font):
        ...
        taba.set_tab(0,Pango.TabAlign.LEFT,50)
        taba.set_tab(1,Pango.TabAlign.LEFT,200)
        taba.set_tab(2,Pango.TabAlign.LEFT,350)
        taba.set_tab(3,Pango.TabAlign.LEFT,466)
        layout.set_tabs(taba)
        ...
        taba.set_tab(0,Pango.TabAlign.LEFT,26)
        taba.set_tab(1,Pango.TabAlign.LEFT,106)
        taba.set_tab(2,Pango.TabAlign.LEFT,146)
        layout.set_tabs(taba)
        for i in range(12):
    ...
    def main_labels(self,cr,font,kind='radix'):
        ...
        if kind == 'local':
            ...
            if boss.opts.lang == 'ca' and curr.curr_chart.country == u'España':
                region = cata_reg[region]
            layout.set_text(curr.curr_chart.city+' ('+region+'-'+t(curr.curr_chart.country)[0]+')')
            ink,logical = layout.get_extents()
            xpos = logical.width/Pango.SCALE
