...
class Basic_Chart(object):
    ...
    def sortplan(self):
        ...
        return sorted(pl, key=lambda d: d['degree'])
...
class HouseChart(Basic_Chart,EqualHousesChart):
    ...
    def get_sign_offsets(self):
        for long,size in zip(reversed(self.get_sign_cusps()),reversed(self.chart.sign_sizes())):
            off = 180 - long
            yield (off + (90 - size/2))
...
class SubjectClickChart(RadixChart):
    ...
    def subject_sign_offsets(self):
        for long,size in zip(reversed(self.get_sign_cusps()),reversed(self.subject_sizes())):
            off = 180 - long
            yield (off + (90 - size/2))
