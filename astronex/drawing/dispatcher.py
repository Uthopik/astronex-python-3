...
from astronex.drawing.progsheet import ProgMixin
from astronex.drawing.profchart import ProfileMixin
from astronex.drawing.biograph import BioMixin
from astronex.drawing.diagrams import DiagramMixin
from astronex.drawing.datasheets import SheetMixin
from astronex.drawing.paarwabe import PaarWabeMixin
from astronex.drawing.planetogram import PlanetogramMixin
from astronex.drawing import roundedcharts
...
class DrawMixin(CoreMixin,ProgMixin,ProfileMixin,BioMixin,DiagramMixin,SheetMixin,PaarWabeMixin,PlanetogramMixin):
    ...
    def draw_transits(self,cr,width,height,chartob=None):
        ...
        if showEA:
            ...
            over = target.create_similar(cairo.CONTENT_COLOR_ALPHA,int(width),int(height))
    ...
    def get_nw(self,filter=None):
        ...
        if op in ['draw_nat','draw_house','draw_nod','draw_soul','draw_local']:
            ...
            if  not filter:
                return []
            if boss.da.plselpanel is not None and boss.da.plselpanel.get_visible():
                return self.notwanted
            else:
    ...
    def set_op_AP(dispatcher,for_op,state):
        ...
        if not (state & Gdk.ModifierType.CONTROL_MASK):
            ...
            am = -30 if (state & Gdk.ModifierType.SHIFT_MASK) else 30
        ap = (AP_DEG + am) % 360
        AP_DEG = ap
        chartob = dispatcher.get_chartob(for_op) 
        chart = chartob.chart
