import pysw
from astronex import chart
from astronex import database
from collections import deque
from astronex.utils import PersonInfo,dectodeg,parsestrtime
...
class Current(object):
    ...
    def init(self,app):
        ...
        self.pool = deque([])
        pf = path.joinpath(app.home_dir,'mruch.pkl')
        if path.exists(pf):
            try:
                with open(pf, "rb") as f:
                    self.pool = pickle.load(f)
            except Exception as e:
                print("Aviso: no se pudo cargar mruch.pkl (%s). Empezando con pool vacio." % e)
                self.pool = deque([])
        self.couples = []
        self.coup_ix = 0
        pf = path.joinpath(app.home_dir,'coups.pkl')
        if path.exists(pf):
            try:
                with open(pf, "rb") as f:
                    self.couples = pickle.load(f)
            except Exception as e:
                print("Aviso: no se pudo cargar coups.pkl (%s). Empezando con couples vacio." % e)
                self.couples = []
    ...
    def setloc(self,city,code):
        ...
        try:
            fetch(self.country, city, code, self.loc)
    ...
    def chiron(self,ch):
        from astronex.directions import strdate_to_date
        import datetime
        from zoneinfo import ZoneInfo
        dt = strdate_to_date(ch.date)
        dt = datetime.datetime.combine(dt.date(),dt.time())
        nxdate = NeXDate(self,dt,ZoneInfo(ch.zone))
